...
public class AppController {
    ...
    @Cacheable(
            value = "good_app_page",
            key = "T(com.lanhai.lanaicodemother.utils.CacheKeyUtils)." + "generateKey(#appQueryRequest)",
    ...
    public BaseResponse<Boolean> canDownloadAppCode(@PathVariable Long appId,
        ...
        String sourceDirPath = AppConstant.CODE_OUTPUT_ROOT_DIR + File.separator + sourceDirName;
        File sourceDir = new File(sourceDirPath);
        boolean codeDirExists = sourceDir.exists() && sourceDir.isDirectory();
        // 只有静态资源存在且代码目录存在时，才允许下载
        boolean canDownload = staticResourceExists && codeDirExists;
        return ResultUtils.success(canDownload);
    }
     ...
     * @return 基础URL（如：http://localhost:8123/api）
     */
