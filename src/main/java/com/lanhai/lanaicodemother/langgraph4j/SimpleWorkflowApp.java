...
public class SimpleWorkflowApp {
    ...
    /**
     * 这是一个静态方法，用于创建一个异步节点操作
     *
     * @param message 要处理的节点消息内容
     * @return 返回一个AsyncNodeAction类型的异步节点操作对象
     * 该对象封装了节点的执行逻辑和状态处理
     * 创建工作节点的通用方法
     */
    static AsyncNodeAction<MessagesState<String>> makeNode(String message) {
        return node_async(state -> {  // 记录节点执行的日志信息
            log.info("执行节点: {}", message); // 返回包含消息的Map对象
